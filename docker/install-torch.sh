...
#
# Usage: install-torch.sh <venv> <index-url> <constraints-out>
# Candidates come from TORCH_CANDIDATES (newline- or '|'-separated), newest
# first; the last one should be a known-good fallback.

set -uo pipefail

VENV="${1:?usage: install-torch.sh <venv> <index-url> <constraints-out>}"
INDEX="${2:?missing index url}"
CONSTRAINTS="${3:?missing constraints output path}"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PY="$VENV/bin/python"

: "${TORCH_CANDIDATES:=torch==2.11.0 torchvision==0.26.0 torchaudio==2.11.0
...
say "index: $INDEX"
say "available torch versions on this index"
# Purely informational: if every candidate fails, this listing is what tells
# us what to pin instead, without another round-trip through CI.
...
if [ -z "$winner" ]; then
    say "FAILED: no candidate produced a torch usable on sm_120 + sm_89"
    echo "Pin a known-good combination via the TORCH_CANDIDATES build arg."
    echo "The version listing above shows what this index actually offers."
    exit 1
fi
